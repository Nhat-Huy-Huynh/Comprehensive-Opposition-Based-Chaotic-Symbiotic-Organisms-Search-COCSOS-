...
def load_seed_table_from_cocsos_excel(seed_excel_file, num_runs, function_name="Relay_TDS_2D_plus_2"):
    """
    Đọc seed do COCSOS xuất ra Excel.
    Ưu tiên sheet Function_Seeds giống benchmark/COCSOS seed protocol.
    """
    candidate_sheets = ["Function_Seeds", "Run_Function_Seeds", "Seed_Protocol"]
    last_error = None

    for sheet in candidate_sheets:
        try:
            seed_table = pd.read_excel(seed_excel_file, sheet_name=sheet)
            seed_table = check_seed_table(
                seed_table,
                num_runs=num_runs,
                function_name=function_name,
                source_sheet=sheet
            )
            print(f"Đã đọc seed từ file '{seed_excel_file}', sheet '{sheet}'.")
            return seed_table
        except Exception as exc:
            last_error = exc
    ...
    raise ValueError(
        ...
        "Cần có một trong các sheet: Function_Seeds, Run_Function_Seeds, Seed_Protocol. "
        f"Lỗi cuối cùng: {last_error}"
    )


def check_seed_table(seed_table, num_runs, function_name="Relay_TDS_2D_plus_2", source_sheet=None):
    ...
    # Lọc đúng Function để tránh bị ảnh hưởng nếu file Excel có thêm bài toán/hàm khác.
    seed_table = seed_table[seed_table["Function"] == str(function_name)].copy()
    ...
    if missing_runs:
        raise ValueError(
            f"seed_table thiếu seed cho các Run: {sorted(missing_runs)} "
            f"| Function={function_name} | Sheet={source_sheet}"
        )
...
def build_bounds(bounds):
    """
    Tạo lb/ub cho bài toán 2D+2.
    Phần PS dưới đây được đồng bộ theo bản COCSOS_2Dplus2_seed_protocol để khi dùng cùng seed,
    SOS sinh đúng cùng quần thể ban đầu với COCSOS.
    """
...
def build_initial_population_dataframe(
    ...
    - Initial Quantized OF: giá trị hàm mục tiêu dùng để so sánh công bằng với COCSOS/SOS.
    """
...
def SOS(obj_func, bounds, dim, pop_size=100, max_iter=3000, seed=None):
    ...
    lb, ub = build_bounds(bounds)

    # ==============================================================
    # 1) Khởi tạo quần thể ngẫu nhiên ban đầu bằng seed đọc từ COCSOS
    # ==============================================================
...
def export_results_to_excel(
    ...
    df_summary = pd.DataFrame({
        ...
        "Seed": [seed],
        "Seed Excel Source": [seed_excel_file],
        "Dimension": [dim],
        "Pop Size": [pop_size],
        "Max Iter": [max_iter],
        "Bounds": [str(bounds)],
    ...
    seed_protocol_df = pd.DataFrame({
        ...
        "Function": [function_name],
        "Seed": [seed],
        "Seed Source": ["Read from COCSOS Excel Function_Seeds"],
        "Seed Excel Source": [seed_excel_file],
        "Dimension": [dim],
        "Pop Size": [pop_size],
        "Max Iter": [max_iter],
        ...
        "Random reset": ["np.random.seed(seed) and random.seed(seed) before np.random.uniform"]
    })
...
# ------------------ Chạy thuật toán SOS nhiều lần, đọc seed từ COCSOS và xuất Excel ------------------
if __name__ == "__main__":
    ...
    # File Excel tổng do COCSOS_2Dplus2_seed_protocol.py tạo ra.
    cocsos_seed_excel_file = "COCSOS_110kV_newPS_newAB_chung_seed_protocol_ALL_SUMMARY.xlsx"
    excel_filename_prefix = "SOS_110kV_TOF_newPS_newAB_chung_seed"

    seed_table = load_seed_table_from_cocsos_excel(
    ...
    experiment_info_df = pd.DataFrame({
        ...
        "Value": [
            ...
            int(pop_size),
            int(max_iter),
            f"Read from COCSOS Excel file: {cocsos_seed_excel_file}, sheet Function_Seeds",
            "Each SOS Run uses the same Run + Function seed generated by COCSOS.",
            ...
            "COCSOS and SOS must use same seed, same dim=2D+2, same bounds/lb/ub/pop_size, same quantize_solution and same objective function."
        ]
    })
    ...
    for run in range(1, num_runs + 1):
        ...
        summary_record = {
            ...
            "Function": function_name,
            "Seed": function_seed,
            "Seed Excel Source": cocsos_seed_excel_file,
            "Dimension": total_dim,
            "Pop Size": pop_size,
            "Max Iter": max_iter,
            "Bounds": str(bounds),
    ...
    with pd.ExcelWriter(all_file_name) as writer:
        ...
        seed_table.to_excel(writer, sheet_name="Function_Seeds", index=False)
        seed_table.to_excel(writer, sheet_name="Run_Function_Seeds", index=False)
        experiment_info_df.to_excel(writer, sheet_name="Experiment_Info", index=False)

    print(f"\nAll SOS runs summary exported to {all_file_name}")
