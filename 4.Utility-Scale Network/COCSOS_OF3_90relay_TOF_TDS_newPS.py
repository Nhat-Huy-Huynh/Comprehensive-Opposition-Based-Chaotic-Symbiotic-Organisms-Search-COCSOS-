...
def check_constraints(x, iteration_count=0):
    # Đối với phiên bản alpha_constant:
    alpha_backup = 5000
...
def objective_function(x,iteration_count=0):
    ...
    for i in range(D):
        primary_obj += (
            0.2 * primary_times[i] +
            0.6 * primary_times_mid[i] +
            0.2  * primary_times_far[i]
        ) *p_jump[i]
...
def chaotic_local_search(best_solution, best_fitness, pop, obj_func, lb, ub, local_search_limit=20):
    ...
    for _ in range(local_search_limit):
        ...
        new_sol = quantize_solution(new_sol,lb,ub)
...
def make_run_seed_table(num_runs=30, function_name="Relay_TDS"):
    """
...
def check_seed_table(seed_table, num_runs, function_name="Relay_TDS"):
...
def get_run_seed(seed_table, run, function_name="Relay_TDS"):
...
def build_initial_population_dataframe(initial_info, run, function_name="Relay_TDS", population_key="Initial Random Population"):
    """
...
def COCSOS(obj_func, bounds, dim, pop_size=100, max_iter=3000, seed=None):
    ...
    D_local = dim // 2
    lb = np.concatenate(
        (np.array([bounds[0]] * D_local), # TDS thấp nhất
    ...
    ub = np.concatenate(
        (np.array([bounds[1]] * D_local), # TDS cao nhất
    ...
    for iteration in range(1, max_iter+1):
        ...
        if np.random.rand() < 0.4:
            ...
            fitness_combined = np.array([obj_func(ind,iteration) for ind in combined])
...
def export_results_to_excel(
    ...
    seed_protocol_df = pd.DataFrame({
        ...
        "Reproducibility condition": ["SOS must use the same Run + Function seed before np.random.uniform(lb, ub, (pop_size, dim))."]
    })
    ...
    experiment_info_df = pd.DataFrame({
        ...
        "Value": [
            ...
            "Iteration = 0 stores Initial Best OF before CO population.",
            "SOS must read the same seed and must not call any random function before creating the initial population."
        ]
    })
...
# ------------------ Chạy thuật toán COSOS 30 lần và xuất ra Excel ------------------
if __name__ == "__main__":
    bounds = (0.05, 3)  # Biên cho TDS
    pop_size = 100
    ...
    # COCSOS chạy trước nên tự sinh bảng seed và lưu lại để SOS đọc đúng cùng seed.
    ...
    # Chạy 30 lần và xuất kết quả ra 30 file Excel riêng biệt
    for run in range(1, num_runs + 1):
        function_seed = get_run_seed(seed_table, run, function_name=function_name)

        print(f"\n======== RUN {run} / {num_runs} | Seed = {function_seed} ========")
        ...
        filename = f"COCSOS_OF3_110kV_TDS_newPS_VI_{run}.xlsx.xlsx"
    ...
    all_file_name = "COCSOS_OF3_110kV_TDS_newPS_VI_ALL_SUMMARY.xlsx"
