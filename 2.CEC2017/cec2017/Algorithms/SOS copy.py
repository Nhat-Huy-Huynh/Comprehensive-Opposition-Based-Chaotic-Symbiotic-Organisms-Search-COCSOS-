...
def random_select(pop, i):
    """Chọn ngẫu nhiên một cá thể khác i trong quần thể."""
    candidates = [k for k in range(len(pop)) if k != i]
    return np.random.choice(candidates)


# ============================================================
# 2. CƠ CHẾ SEED: ĐỌC LẠI SEED TỪ FILE EXCEL COCSOS SUMMARY
# ============================================================
...
def read_seed_table_from_cocsos_excel(seed_excel_file, num_runs, function_names):
    """
    Đọc seed từ file Excel summary của COCSOS để SOS dùng lại đúng seed.

    Ưu tiên đọc các sheet theo thứ tự:
    1) Function_Seeds
    2) Run_Function_Seeds
    3) Seed_Protocol
    4) All_Summary
    5) Summary

    File COCSOS summary có thể chứa nhiều function/run hơn phần đang khảo sát;
    hàm này sẽ lọc đúng function_names và Run <= num_runs.
    """
    seed_excel_file = Path(seed_excel_file)
    if not seed_excel_file.exists():
        raise FileNotFoundError(
            f"Không tìm thấy file seed COCSOS summary: {seed_excel_file}\n"
            f"Hãy đặt đúng đường dẫn tại biến COCSOS_SUMMARY_FILE."
        )
    ...
    for sheet in sheet_priority:
        if sheet in available_sheets:
            tmp = pd.read_excel(seed_excel_file, sheet_name=sheet)
            if {"Run", "Function", "Seed"}.issubset(tmp.columns):
                seed_df = tmp[["Run", "Function", "Seed"]].copy()
                used_sheet = sheet
                break

    if seed_df is None:
        raise ValueError(
            "Không tìm được sheet có đủ 3 cột Run, Function, Seed trong file COCSOS summary. "
            f"Các sheet hiện có: {xls.sheet_names}"
        )

    seed_df["Run"] = seed_df["Run"].astype(int)
    seed_df["Function"] = seed_df["Function"].astype(str)
    seed_df["Seed"] = seed_df.apply(
        lambda row: validate_seed(row["Seed"], row["Run"], row["Function"]), axis=1
    ).astype(int)

    # Loại trùng nếu đọc từ All_Summary/Summary/Seed_Protocol có nhiều cột dư.
    seed_df = seed_df.drop_duplicates(subset=["Run", "Function"], keep="first")

    function_names = [str(fn) for fn in function_names]
    seed_df = seed_df[
        (seed_df["Run"].between(1, int(num_runs))) &
        (seed_df["Function"].isin(function_names))
    ].copy()

    seed_df = check_seed_table(seed_df, num_runs=num_runs, function_names=function_names)
    seed_df["Seed Source"] = f"Read from COCSOS summary file: {seed_excel_file.name} | sheet: {used_sheet}"
    return seed_df
...
def check_seed_table(seed_table, num_runs, function_names):
    ...
    if len(seed_table) != expected_rows:
        actual_pairs_preview = seed_table[["Run", "Function"]].head(20).to_dict("records")
        raise ValueError(
            f"seed_table phải có {expected_rows} dòng = num_runs * số_function, "
            f"nhưng sau khi lọc hiện có {len(seed_table)} dòng.\n"
            f"Kiểm tra n_runs, danh sách function đang chạy, và file COCSOS summary.\n"
            f"Một số cặp đang đọc được: {actual_pairs_preview}"
        )
    ...
    if missing_pairs:
        raise ValueError(f"seed_table thiếu cặp Run + Function: {sorted(missing_pairs)[:20]}")

    if seed_table.duplicated(subset=["Run", "Function"]).any():
        duplicated = seed_table[seed_table.duplicated(subset=["Run", "Function"], keep=False)]
        raise ValueError(
            "seed_table bị trùng cặp Run + Function. "
            f"Các dòng trùng đầu tiên:\n{duplicated.head(20)}"
        )
...
# ============================================================
# 3. HÀM HỖ TRỢ XUẤT EXCEL GIỐNG COCSOS
# ============================================================
...
def build_experiment_info_df(num_runs, pop_size, max_iter, seed_excel_file):
    ...
    return pd.DataFrame({
        ...
        "Value": [
            ...
            int(pop_size),
            int(max_iter),
            f"Read from COCSOS summary Excel file: {Path(seed_excel_file).name}",
            "Each Run + Function pair reuses the seed recorded by COCSOS.",
            "np.random.seed(seed) and random.seed(seed)",
            "Initial random population and Initial Random OF are saved immediately after random initialization.",
            "To reproduce the same initial population as COCSOS, keep the same seed, Function, Run, bounds, dimension, population size, and np.random.uniform(lb, ub, (pop_size, dim)) formula.",
            "Comprehensive Opposition initialization/phase and Chaotic Local Search are removed.",
            "Mutualism, Commensalism, and Parasitism phases are kept as the original SOS procedure."
        ]
    })


# ============================================================
# 4. THUẬT TOÁN SOS GỐC
#    - Bỏ CO population
#    - Bỏ Opposition Phase
#    - Bỏ Chaotic Local Search CLS
#    - Giữ 3 pha: Mutualism, Commensalism, Parasitism
# ============================================================

def SOS(obj_func, bounds, dim, pop_size=50, max_iter=1000, seed=None):
    """
    SOS gốc dùng seed đọc từ COCSOS để khởi tạo cùng quần thể ban đầu.

    Điểm quan trọng để so sánh công bằng:
    - Sau khi set seed, lệnh đầu tiên dùng np.random là:
      np.random.uniform(lb, ub, (pop_size, dim))
    - Vì vậy quần thể ngẫu nhiên ban đầu giống COCSOS trước khi COCSOS áp dụng CO.
    """
    if seed is not None:
    ...
    # ==============================================================
    # 1) Khởi tạo dân số ngẫu nhiên ban đầu giống COCSOS
    # ==============================================================
    ...
    # SOS gốc bắt đầu trực tiếp từ quần thể ngẫu nhiên ban đầu.
    pop = pop_random_initial.copy()
    fitness = fitness_random_initial.copy()

    best_sol = initial_best_sol.copy()
    best_fit = initial_best_fit

    # Iteration 0 là trạng thái sau khởi tạo ngẫu nhiên ban đầu.
    ...
    for iteration in range(1, max_iter + 1):
        for i in range(pop_size):
            ...
            new_ind = pop[i] + r1 * (best_sol_current - pop[j])
...
# ============================================================
# 5. CHẠY SOS CHO TẤT CẢ FUNCTION VÀ XUẤT EXCEL GIỐNG COCSOS
# ============================================================
...
def run_sos_for_all(
    ...
    pop_size=50,
    max_iter=1000,
    excel_filename_prefix="SOS_CEC2017_F1-F10_30dim",
    ...
    for name, (func, bound, global_min, dim) in functions.items():
        ...
        best_sol, best_val, elapsed_time, best_history, initial_info = SOS(
            obj_func=func,
            bounds=bound,
            dim=dim,
            pop_size=pop_size,
        ...
        seed_records_this_run.append({
            ...
            "Function": name,
            "Seed": function_seed,
            "Seed Source": "Read from COCSOS summary Excel",
            "Dimension": dim,
            "Bounds": str(bound),
            "Pop Size": pop_size,
            "Max Iter": max_iter,
...
def run_multiple_times(
    ...
    max_iter=1000,
    excel_filename_prefix="SOS_CEC2017_F1-F10_30dim",
    seed_excel_file="COCSOS_CEC2017_F1-F10_30dim_ALL_SUMMARY.xlsx",
):
    function_names = list(functions.keys())

    # SOS không tự sinh seed mới. SOS đọc lại seed từ file summary của COCSOS.
    seed_table = read_seed_table_from_cocsos_excel(
        seed_excel_file=seed_excel_file,
        num_runs=n_runs,
        function_names=function_names,
    )
    experiment_info_df = build_experiment_info_df(n_runs, pop_size, max_iter, seed_excel_file)

    all_summary_records = []
    all_seed_records = []

    for run_i in range(1, n_runs + 1):
        print(f"\n--- Starting SOS run {run_i} ---")
        summary_df, seed_df = run_sos_for_all(
            functions,
            run_i,
    ...
    function_seed_df["Seed Source"] = f"Read from COCSOS summary Excel: {Path(seed_excel_file).name}"
    ...
    print(f"All {n_runs} SOS runs completed.")
    print(f"All SOS CEC2017 runs summary exported to {all_file_name}")
    return function_seed_df


# ============================================================
# 6. MAIN: CẤU HÌNH GIỐNG FILE COCSOS
# ============================================================

if __name__ == "__main__":
    max_iter = 1000

    # File ALL_SUMMARY do COCSOS xuất ra.
    # Đổi đường dẫn này theo đúng vị trí file của bạn.
    COCSOS_SUMMARY_FILE = r"COCSOS_CEC2017_F11-F20_100dim_ALL_SUMMARY.xlsx"
    ...
    # Dictionary chứa các hàm benchmark.
    functions = {}
    ...
    run_multiple_times(
        ...
        pop_size=50,
        max_iter=max_iter,
        excel_filename_prefix="SOS_CEC2017_F11-F20_100dim",
        seed_excel_file=COCSOS_SUMMARY_FILE,
    )
