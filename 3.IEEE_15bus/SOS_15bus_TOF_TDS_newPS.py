...
# ------------------ Thiết lập chung phải giống COCSOS để tái lập quần thể ban đầu ------------------
...
def quantize_solution(x):
    ...
    ps_quant = np.clip(np.round(ps_cont * 100000) / 100000, PS_LOWER_BOUND, PS_UPPER_BOUND)
...
def read_cocsos_seed_table(cocsos_seed_excel, num_runs=None):
    """
    Đọc bảng seed do COCSOS đã xuất ra Excel.
    ...
    if not os.path.exists(cocsos_seed_excel):
        raise FileNotFoundError(
            ...
            f"Hãy đặt file này cùng thư mục với code SOS hoặc sửa biến COCSOS_SEED_EXCEL trong __main__."
        )
...
def make_initial_population_df(initial_info, run, seed, dim):
    """
    Tạo bảng quần thể khởi tạo ngẫu nhiên ban đầu.
    Lưu cả nghiệm và OF tương ứng để so sánh với COCSOS.
    """
...
# ------------------ Thuật toán SOS có dùng seed từ COCSOS ------------------
def SOS(obj_func, bounds, dim, pop_size=50, max_iter=5000, seed=None, run=None):
    """
    Điểm quan trọng để công bằng với COCSOS:
    - Nếu seed != None, reset np.random và random trước khi khởi tạo.
    - Dùng cùng bounds TDS, PS bounds, dim, pop_size, quantize_solution.
    - Lưu lại quần thể khởi tạo ban đầu và OF.
    """
    if seed is not None:
    ...
    D_local = dim // 2
    lb = np.concatenate((np.array([bounds[0]] * D_local), np.array([PS_LOWER_BOUND] * D_local)))
    ub = np.concatenate((np.array([bounds[1]] * D_local), np.array([PS_UPPER_BOUND] * D_local)))

    # ==============================================================
    # 1) Khởi tạo quần thể ngẫu nhiên ban đầu bằng đúng seed của COCSOS Run hiện tại
    # ==============================================================
    ...
    # SOS gốc bắt đầu trực tiếp từ quần thể khởi tạo, không có CO phase.
...
def export_results_to_excel(
    ...
    df_seed = pd.DataFrame({
        ...
        "Reproducibility condition": [
            "Use the same Run seed, bounds, dim, pop_size, quantize_solution, objective_function, and relay data as COCSOS."
        ]
    })
    ...
    df_experiment = pd.DataFrame({
        ...
        "Value": [
            ...
            "Initial_Random_Pop",
            "Objective function value of each individual in the initial random population before SOS updates.",
            "np.random.seed(seed) and random.seed(seed)",
            "TDS/PS bounds and quantize_solution are matched to the COCSOS seed-protocol code so that the initial population is reproducible."
        ]
    })
...
# ------------------ Chạy SOS nhiều lần, đọc seed từ Excel của COCSOS và xuất Excel ------------------
if __name__ == "__main__":
    ...
    num_runs = 20         # Nên bằng số run của COCSOS

    sos_excel_filename_prefix = "SOS_15bus_TOF_TDS_newPS_EI_seed_protocol"

    # File tổng hợp COCSOS đã xuất ra từ code COCSOS seed protocol.
    COCSOS_SEED_EXCEL = "COCSOS_15bus_TOF_TDS_newPS_EI_seed_protocol_ALL_SUMMARY.xlsx"
    ...
    experiment_info_df = pd.DataFrame({
        ...
        "Value": [
            ...
            "Use the same Run seed, TDS/PS bounds, dim, pop_size, objective_function, and quantize_solution as COCSOS."
        ]
    })
