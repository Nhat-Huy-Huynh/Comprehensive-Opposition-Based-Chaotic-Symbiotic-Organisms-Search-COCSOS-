...
The values of TDS and PS must remain within their corresponding limits throughout the optimization process.
...
\left[
\frac{A_i}
{\left(\dfrac{I_i}{I_{\mathrm{pickup},i}}\right)^{B_i}-1}
\right],
]
...
\left[
\frac{0.14}
{\left(\dfrac{I}{I_{\mathrm{pickup}}}\right)^{0.02}-1}
\right].
]
...
\left[
\frac{13.5}
{\left(\dfrac{I}{I_{\mathrm{pickup}}}\right)-1}
\right].
]
...
\left[
\frac{80}
{\left(\dfrac{I}{I_{\mathrm{pickup}}}\right)^2-1}
\right].
]

The supplied EI source file uses:
...
In addition to TDS and PS, the characteristic parameters (A) and (B) are optimized. The operating time is calculated as:
...
A typical decoding rule is:

```python
curve_code = np.clip(np.rint(curve_variable), 1, 3).astype(int)
```
...
The characteristic constants are then assigned as follows:

| Characteristic | (A_i) | (B_i) |
| -------------- | ----: | ----: |
| NI             |  0.14 |  0.02 |
| VI             |  13.5 |  1.00 |
| EI             |  80.0 |  2.00 |
...
A corresponding Python implementation is:
...
For every primary–backup pair (k), the backup relay must operate after the primary relay by at least the Coordination Time Interval:
...
The supplied problem uses:

[
CTI=0.2\ \text{s}.
]

The coordination violation is calculated as:
...
## 10. Seed Protocol Across the Five Cases
...
This ensures that the population generated for a run can be reproduced when the same:
...
### 10.2. Fair Comparison Between Cases
...
## 11. Current Official Run Settings
...
> **Note:** The argument `bounds=(0.1, 1.1)` commonly represents the normalized or general optimizer bounds used by the supplied implementation. The actual TDS and PS limits must still be handled according to the variable-group mapping implemented in the objective function or population-decoding function.

---

## 12. Per-Run Output Files
...
## 14. Input Data Included in the Code
...
## 15. Direct Use of the Optimizer
